...
const MessageBubble = React.memo(({ message, isLeft, uiConfig, onContentReady, isLoading, isVisible }: {
  ...
  return (
    <div className={`${roundedClass} ${message.type === 'image' ? 'p-1' : 'p-3'} ${message.maxWidth || 'max-w-xs'} border-solid relative`} style={bubbleStyle}>
      ...
      <AnimatePresence mode="wait">
        {showAvatar ? (
          <motion.img key="avatar" src={person.avatar} alt={person.name}
            ...
            initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} transition={{ duration: 0.2 }} />
...
const ChatComponent: React.FC<ChatComponentProps> = ({ config, uiConfig = {} }) => {
  ...
  return (
    <div key={key} className="relative w-full rounded-lg overflow-hidden" style={{ height: `${ui.containerHeight}px` }}>
      <div className="absolute top-0 left-0 right-0 h-16 pointer-events-none z-10" style={{ background: topFade }} />
      <div ref={containerRef} className="p-4 h-full overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
